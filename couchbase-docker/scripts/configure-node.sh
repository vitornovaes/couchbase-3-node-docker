...
/opt/couchbase/bin/couchbase-cli node-init --cluster localhost:8091 -u Administrator --password password --node-init-data-path='/opt/couchbase/var/lib/couchbase/data' --node-init-index-path='/opt/couchbase/var/lib/couchbase/indexes' --node-init-hostname='127.0.0.1' > /dev/null
...
if [[ "${NODE_TYPE}" == "DEFAULT" ]]; then
  ...
  /opt/couchbase/bin/couchbase-cli setting-cluster --cluster localhost:8091 -u $CLUSTER_USERNAME --password $CLUSTER_PASSWORD --cluster-name "couch" > /dev/null
  ...
  CMD="$CMD --cluster localhost:8091"
  CMD="$CMD -u $CLUSTER_USERNAME"
  CMD="$CMD -p $CLUSTER_PASSWORD"
...
/opt/couchbase/bin/couchbase-cli server-add --cluster couchbase-node1:8091 -u Administrator -p password --server-add http://"`hostname -I|sed 's/ *$//g'`:8091" --server-add-username Administrator --server-add-password password --services $SERVICES

  if [[ "${REBALANCE}" == "1" ]]; then
    echo Rebalancing Cluster
    if (/opt/couchbase/bin/couchbase-cli rebalance-status --cluster couchbase-node1:8091 -u $CLUSTER_USERNAME -p  $CLUSTER_PASSWORD | grep -q running) then
      echo Only one rebalance operation can be done at a time, waiting for the current rebalance to complete
      until $(couchbase-cli rebalance-status --cluster couchbase-node1:8091 -u $CLUSTER_USERNAME -p $CLUSTER_PASSWORD | grep -q notRunning); do
        echo .
        sleep 5
      done
    fi
    /opt/couchbase/bin/couchbase-cli rebalance \
      --cluster couchbase-node1:8091 \
      -u $CLUSTER_USERNAME \
      -p $CLUSTER_PASSWORD \
    > /dev/null
  fi
